...
current_id = 0
label_ids = {}#directory for labels
y_labels = []#actual labels
x_train = []#pixel values
#նկարները տեսնելու համար
for root, dirs, files in os.walk(image_dir):
 for file in files:
  if file.endswith("png") or file.endswith("jpg"):
   path = os.path.join(root, file)
   label = os.path.basename(root).replace(" ", "-").lower()#label-ին տալիս ենք դիրեկտորիայի անունը
   #print(label, path)
